...
* `objectFactoryEnabled` (Experimental): If `true`, generates Java source code for the populated object in the
  ...
  **Note**: This will not work if the `FIELD` strategy is enabled.
...
* **Conceptual Equivalence**: The generated Java code is conceptually equivalent to the population behavior used at runtime. For example, if an object is populated using a private constructor via reflection, the code generation engine mirrors this behavior by outputting a reflection-based instantiation helper rather than trying to construct it using standard direct constructors.
