...
public class ConstructorPopulator implements PopulatingStrategy {
    ...
    public <T> T populate(ClassCarrier<T> classCarrier, Populator populator) {
        return continuePopulateUsingConstructor(classCarrier, populator);
    }

    private <T> T continuePopulateUsingConstructor(ClassCarrier<T> classCarrier, Populator populator) {
        ...
        try {
            ...
            return continuePopulateUsingConstructor(constructor, classCarrier, populator);
        } catch (Exception e) {
            throw new PopulateException(format(FAILED_TO_CREATE_OBJECT, clazz.getName(), CONSTRUCTOR), e);
        }
    }

    protected <T> T continuePopulateUsingConstructor(Constructor<T> constructor, ClassCarrier<T> classCarrier, Populator populator) throws InstantiationException, IllegalAccessException, InvocationTargetException {
